...
for d in ['BASE','RP','PCA','ICA','RF','SVD','img']:
    n = './{}/'.format(d)
    if not os.path.exists(n):
        os.makedirs(n)

OUT = './BASE/'
madX1 = pd.read_csv('./madelon/madelon_train.data',header=None,sep=' ')
madX2 = pd.read_csv('./madelon/madelon_valid.data',header=None,sep=' ')
madX = pd.concat([madX1,madX2],0).astype(float)
madY1 = pd.read_csv('./madelon/madelon_train.labels',header=None,sep=' ')
madY2 = pd.read_csv('./madelon/madelon_valid.labels',header=None,sep=' ')
madY = pd.concat([madY1,madY2],0)
madY.columns = ['Class']

madelon_trgX, madelon_tstX, madelon_trgY, madelon_tstY \
    = ms.train_test_split(madX, madY,
                          test_size=0.2,
                          random_state=0,
                          stratify=madY)
